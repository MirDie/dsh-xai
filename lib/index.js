...
*/
function createXaiOAuthAdapter(session, resolveAttachments, reasoning) {
...
const inject = ["llm"];
const REASONING_LEVELS = Object.keys({
	off: true,
	minimal: true,
	low: true,
	medium: true,
...
const Config = z.object({ reasoning: z.union(REASONING_LEVELS) });
/**
...
function apply(ctx, config) {
	...
	ctx.llm.registerAdapter([XAI_OAUTH_ROUTE], createXaiOAuthAdapter(session, () => ctx.get("attachments"), config.reasoning));
